...
#     met_mask = (all_model_norms.transpose()[0,:] == solar_met) #Takes first column and selects metallicity = 0.02
    ...
    if nssps == 3:
        ## Just adding this conditional for the legac histogram plots - to add to the nasa ppt presentation (excluding old SSPs so we set nssps=3)
...
def plot_stellar_mass_fractions(cfitfile, nssps=4):
    ...
    #(the age_of_universe is for the LEGA-C galaxy spectra - legac_M13_230105_v2.0 at z=1.17 -- used this info to add to nasa symposium ppt presentation )
    total_mass = np.sum(a_i,axis=1) # np.sum(np.sum(a_i,axis=1))
    age_of_universe =  5.13000000e+09
    avg_star_formation = total_mass/age_of_universe
    print("Total mass: ", total_mass, " solar masses\nAverage star formation rate: ", avg_star_formation, " solar masses per year")
...
#     plot_stellar_mass_fractions(datfile)
    plot_stellar_mass_fractions(datfile, nssps=3) #use nssps = 4 in general 
